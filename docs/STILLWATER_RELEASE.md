...
- Photos uses a varied, rotated masonry collection. Undo restores image position, favorite and photo wallpaper. Wallpaper state accounts for the current scene.
- GitHub profile/repositories render before optional stars finish loading. Timeouts, partial-data warnings, refresh errors and full-collection links are explicit.
...
`npm test`: 35 passed. `npm run build`: passed. `npm run lint`: no errors or rule warnings. `git diff --check`: passed.
...
2. Settings: scrub dawn/day/evening/night, reset to local time, cross midnight or change the device time zone, suspend/resume the tab, toggle motion and low power, inspect both hemispheres. Confirm smooth colour transitions, no shader errors and a usable fallback without WebGL.
