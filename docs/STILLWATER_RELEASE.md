...
Night-water follow-up: the first coastal shader omitted the meteor from its reflection pass and sampled only the sparse star layer at low intensity. Its uncompressed mirror also cropped stars above normalized sky height 0.86. Sky and water now sample one shared field containing both star layers and the same live meteor head, tail and fade. The full visible sky projects into the bay, with the highest stars above the bottom edge. Pixel-aware star filtering, a short horizontal blur and a nonzero trough brightness keep the glints visible through ripples. Only celestial light is reflected; the dark water palette and shoreline occlusion remain. This correction passed offline shader language/link validation and the production build; its live visual appearance still requires device acceptance.
