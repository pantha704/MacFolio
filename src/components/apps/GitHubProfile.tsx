...
const githubUrl = (value: unknown) =>
  typeof value === 'string' && /^https:\/\/github\.com\//.test(value)
    ? value
    : profile.github
  ...
  if (!response.ok)
    throw new Error(
      response.status === 403 || response.status === 429
        ? 'GitHub’s public request limit was reached.'
...
export default function GitHubProfile() {
  ...
  const [tab, setTab] = useState<'overview' | 'repositories' | 'stars'>(
    'overview',
  )
  ...
  useEffect(() => {
    ...
    const starredRequest = getJson<GitHubRepo[]>(
      'https://api.github.com/users/pantha704/starred?sort=created&per_page=10',
      controller.signal,
    )
      .then((repos) => {
        if (!Array.isArray(repos)) throw new Error('Invalid starred collection')
        return repos
      })
      .catch(() => {
    ...
    const profileRequest = Promise.all([
      getJson<GitHubUser>(
        'https://api.github.com/users/pantha704',
        controller.signal,
      ),
      getJson<GitHubRepo[]>(
        'https://api.github.com/users/pantha704/repos?sort=updated&per_page=100',
        controller.signal,
      ),
    ])
      .then(([user, repos]) => {
        if (!user?.login || !Array.isArray(repos))
          throw new Error('GitHub returned an unexpected response.')
        if (!active) return
        memoryCache = {
          user: { ...user, html_url: githubUrl(user.html_url) },
          repos: repos
  ...
  if (!data && loading)
    return (
      <div className="github-state" role="status">
  ...
  if (!data)
    return (
      <div className="github-state" role="alert">
        <h2>GitHub couldn’t load right now.</h2>
        <p>{error || 'The connection may be unavailable.'}</p>
        <div>
          <button
  ...
  return (
    <div className="github-app">
      {error && (
        <p className="github-note" role="status">
          {error} Showing the last loaded profile.{' '}
          <button
            className="secondary-action"
            onClick={() => setAttempt((value) => value + 1)}
          >
      ...
      <header className="github-profile">
        ...
        <div>
          ...
          <div className="github-meta">
            <span>
              <Users size={15} />
              <strong>{user.followers}</strong> followers · {user.following}{' '}
              following
            </span>
            {user.location && (
              <span>
      ...
      <nav className="github-tabs" aria-label="GitHub sections">
        ...
        <button
          aria-current={tab === 'stars' ? 'page' : undefined}
      ...
      {tab === 'overview' ? (
        <section className="github-content">
          ...
          <RepoGrid repos={data.repos.slice(0, 6)} />
          {starsError && (
            <p className="github-note">
              Starred repositories couldn’t be refreshed, but the profile and
        ...
        <section className="github-content">
          <label className="github-search">
            ...
            <input
              ...
              placeholder={`Find ${tab === 'stars' ? 'a starred repository' : 'a repository'}…`}
            />
          </label>
          ...
          <p className="github-note">
            {tab === 'stars'
              ? 'Showing the 10 most recently starred repositories.'
              : `Searching ${data.repos.length} recently updated repositories of ${user.public_repos}.`}{' '}
            <a
              href={
                tab === 'stars'
                  ? `${profile.github}?tab=stars`
                  : `${profile.github}?tab=repositories`
              }
              target="_blank"
              rel="noopener noreferrer"
            >
...
function RepoList({ repos }: { repos: GitHubRepo[] }) {
  if (!repos.length)
    return <p className="empty-state">No matching repositories.</p>
  return (
