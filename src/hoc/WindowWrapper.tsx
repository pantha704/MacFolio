...
function defaultRect(key: WindowKey, viewport: Viewport): WindowRect {
  ...
  try {
    const saved = JSON.parse(
      safeStorage.getItem(geometryKey(key)) ?? 'null',
    ) as Partial<WindowRect> | null
    if (
      saved &&
      ['left', 'top', 'width', 'height'].every((part) =>
        Number.isFinite(saved[part as keyof WindowRect]),
      )
    )
      return clampWindow(saved as WindowRect, viewport.width, viewport.height)
  } catch {
    /* A corrupt layout falls back to a centered window. */
  }
...
const WindowWrapper = (
  ...
  const Wrapped = (props: Record<string, unknown>) => {
    ...
    const [rect, setRect] = useState<WindowRect>(() =>
      defaultRect(windowKey, viewportNow()),
    )
    ...
    const start = (
      ...
      if (
        ...
        (!target.closest('.window-header') ||
          target.closest(
            'button,input,a,select,textarea,[contenteditable="true"],[data-window-no-drag]',
          ))
      )
        return
    ...
    const geometry = isMobile
      ? {
          ...
          height: Math.max(
            ...
            viewport.height -
              48 -
              (viewport.height < window.innerHeight * 0.75 ? 12 : 78),
          ),
        }
      : item.isMaximized
        ? bounds
    ...
    return (
      <section
        ...
        ref={ref}
        role="region"
        aria-label={appRegistry[windowKey].name}
        aria-hidden={item.isMinimized}
        tabIndex={-1}
        hidden={item.isMinimized}
        className={`window desktop-window${interacting ? ' interacting' : ''}${focused ? ' is-focused' : ''}`}
        style={{
          ...geometry,
          zIndex: item.zIndex,
        ...
        onFocusCapture={() => {
          if (!focused) focusWindow(windowKey)
        }}
        onDoubleClick={(event) => {
          const target = event.target as HTMLElement
          if (
            !isMobile &&
            target.closest('.window-header') &&
            !target.closest('button,input,a,select,[data-window-no-drag]')
          )
            useWindowStore.getState().maximizeWindow(windowKey)
        }}
        onPointerDown={(event) => {
          if (!focused) focusWindow(windowKey)
          start(event, 'move')
        ...
        onPointerMove={(event) => {
          ...
          nextRect.current = clampWindow(
            current.mode === 'move'
              ? {
                  ...current.rect,
                  left: current.rect.left + dx,
                  top: current.rect.top + dy,
                }
              : {
                  ...current.rect,
          ...
          gestureFrame.current = requestAnimationFrame(() => {
            if (nextRect.current) setRect(nextRect.current)
          })
        }}
        onPointerUp={stop}
      ...
      >
        <div className="window-body">
          <Component {...props} windowData={item.data} />
          {!isMobile && !item.isMaximized && (
            <button
              className="window-resize"
              aria-label={`Resize ${appRegistry[windowKey].name}`}
